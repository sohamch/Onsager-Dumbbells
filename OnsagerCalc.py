...
class Interstitial(object):
    ...
    def generateSiteGroupOps(self):
        ...
        groupops = []
        for sites in self.sitelist:
            i0 = sites[0] #the first site in each Wyckoff set.
            oplist = []
            for i in sites:
    ...
    def diffusivity(self, pre, betaene, preT, betaeneT, CalcDeriv=False):
        ...
        for transitionset, rates, symmrates, bET in zip(self.jumpnetwork, ratelist, symmratelist, betaeneT):
            ...
            #     gamma_i[i] = np.dot(pinv2(omega_ij), bias_i[i])
            Dcorr = np.zeros((3,3))
            for i in range(self.N):
        ...
        if not CalcDeriv:
            return D0 + Dcorrection, D0 + Dcorr
        else:
...
class vacancyThermoKinetics(collections.namedtuple('vacancyThermoKinetics',
    ...
    :param pre: prefactors for sites (1/partion_function?)
    :param betaene: energy for sites / kBT
    :param preT: prefactors for transition states (or rates?)
...
class VacancyMediated(object):
    ...
    def __init__(self, crys, chem, sitelist, jumpnetwork, Nthermo=0, NGFmax=4):
        ...
        #invmap stores which Wyckoff set a given site belongs to.
        for ind, w in enumerate(sitelist):
            for i in w:
                self.invmap[i] = ind
        self.om0_jn = copy.deepcopy(jumpnetwork)
        self.GFcalc = self.GFcalculator(NGFmax) #GFCrystalcalc object -> How does it work?
        ...
        # self.kinetic = self.thermo + self.NNstar -> In terms of the states they contain uniquely
    ...
    def GFcalculator(self, NGFmax=0):
        ...
        if NGFmax == getattr(self, 'NGFmax', 0): return self.GFcalc #what does this mean??
        ...
        #Returns a GFCrystalcalc object
    ...
    def generate(self, Nthermo):
        ...
        #Get the omega0 green's function expansion
        ...
        self.clearcache()
        #-> generates the empty dictionaries used in def Lij
    ...
    def Lij(self, bFV, bFS, bFSV, bFT0, bFT1, bFT2, large_om2=1e8):
        ...
        #In __init__ clearcache is called via GFcalculator.
        #In clearcache, the following dicts are declared.
        #Everytime clearcache is called, these dicts are set to empty.
        ...
        if GF is None:
            ...
            GF = np.array([self.GFcalc(PS.i, PS.j, PS.dx)
                           for PS in
                           [self.GFstarset.states[s[0]] for s in self.GFstarset.stars]]) - #stores the Green's functions
                           #from state i to state j based on Block inversion and Taylor Expansion.
                           #Need to do this for g0 and g2 both in case of dumbbells? yes
        ...
        #Energies are computed with respect to the minmum energy
        probVsites *= self.N / np.sum(probVsites)  # normalize - the denom is the partition function
                                                   #The numerator is the number of ways a vacancy can exist in a unit cell.
        probV = np.array([probVsites[sites[0]] for sites in self.sitelist])  # Wyckoff positions
        #Get the symmetry unique site energies
        probVsqrt = np.array([np.sqrt(probV[self.kin2vacancy[starindex]])
                              for starindex in self.vstar2kin])#work this part out
        ...
        prob = np.array([probS[s] * probV[v] for (s, v) in self.kineticsvWyckoff]) #nearest neighbor probability
        for tindex, kindex in enumerate(self.thermo2kin):
        ...
        D0ss = np.dot(self.Dom2, omega2 * symmprobSV2) / self.N #Which equation is this? the uncorrelated diffusivity
        D0sv = -D0ss #why??
