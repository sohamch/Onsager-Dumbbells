...
class StarSet(object):
    ...
    def reIndex43(self, jnet4, jnet3):

        assert len(jnet4) == len(jnet3)

        for jlistInd in range(len(jnet4)):
            assert len(jnet4[jlistInd]) == len(jnet3[jlistInd])
        ...
        for jlistInd in range(len(jnet4)):
            ...
            for jmpInd in range(len(jnet4[jlistInd])):
                om4 = jnet4[jlistInd][jmpInd]
                om3 = jnet3[jlistInd][jmpInd]
                assert om4.state2 == om3.state1
                assert om4.state1 == om3.state2
                pure_ind = self.complexIndexdict[om4.state1][0]
                mixed_ind = self.mixedindexdict[om4.state2][0]
                # omega4 has pure as initial, omega3 has pure as final
                jinitdict4[pure_ind].append(mixed_ind)
                jinitdict3[mixed_ind].append(pure_ind)
                dx = disp4(self.pdbcontainer, self.mdbcontainer, om4.state1, om4.state2)
                new4index.append(((pure_ind, mixed_ind), dx.copy()))
                new3index.append(((mixed_ind, pure_ind), -dx))
                newallindex.append(((pure_ind, mixed_ind), dx.copy()))
                newallindex.append(((mixed_ind, pure_ind), -dx))
        ...
        return symjumplist_omega43_all_indexed,  (symjumplist_omega4_indexed, jtags4), (symjumplist_omega3_indexed, jtags3)