...
class Taylor3D(object):
    ...
    def makeindexPowerYlm(Lmax):
        ...
        # first, the counts
        NYlm = (Lmax + 1) ** 2
        Npower = NYlm + ((Lmax + 1) * Lmax * (Lmax - 1)) // 6 "double / -> integer division"
        # indexing arrays
    ...
    def powexp(cls, u, normalize=True):
        ...
        if umagn < 1e-8: #the origin?
            upow[cls.pow2ind[0, 0, 0]] = 1. #upow stores the products ux^n1*uy^n2*uz^n3
            #where n1+n2+n3 = a given l value <= lmax
            umagn = 0.
        else:
            u0 = u.copy()
            ...
            for n in range(1, cls.Lmax + 1):
                xyz[n, :] = xyz[n - 1, :] * u0[:] #starting from index 1, the row j of xyz represent
            ...
            for n0, n1, n2 in ((n0, n1, n2) for n0 in range(cls.Lmax + 1)
                ...
                #What if i simply wrote ux^n0*uy^n1*uz^n2? -> check - same thing
                #collect the powers to which the components are raised in each row
                #example - if we have ux^1*uy^1*^uz^0
                #then that location in upow, where [110] is stored in ind2pow, is assigned this product
                #See the PowerExpansion module notebook
        if normalize:
            return upow, umagn
        else:
            return upow
    ...
    def scalarproductcoeff(cls, c, a, inplace=False):
        ...
        :param c: scalar *or* dictionary mapping (n,l) to scalars. c-> other in __rmul__
        :param a = list((n, lmax, powexpansion): expansion of function in powers. a-> self in __rmul__
        :param inplace: modify a in place?
        :return coefflist: c*a
        """
        acoeff = getattr(a, 'coefflist', a)
        if inplace:
            # modify the expansion passed in
            ...
            #This is the part that gets executed in line 327 in GFcalc -> omega_Taylor
            else:
                for an, almax, apow in acoeff:
    ...
    def sumcoeff(cls, a, b, alpha=1, beta=1, inplace=False):
        ...
        for bn, blmax, bpow in bcoeff:
            ...
            if not matched:
                ...
                #This is the part that gets executed in defining omega_Taylor in GFcalc - around line 327
                #self.omega_Taylor = sum(symmrate * expansion for symmrate, expansion in zip(self.symmrate, self.Taylorjumps))
