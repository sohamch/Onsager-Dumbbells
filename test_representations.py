...
class SdPair_Tests(unittest.TestCase):
    ...
    def test_jump(self):
        ...
        sefl.assertNotEqual(pair1+j1,pair2w)
        ...
        pair1 = SdPair(0,np.array([1.,0.,0.]),db1)
        j1 = jump(0,0,or_1,or_2,np.array([0.,0.,0.]),np.array([1.,0.,0.]),1,1)
        db2 = (0,or_2,np.array([1.,0.,0.]),1)#dumbbell at the solute site
        pair2 = SdPair(0,np.array([1.,0.,0.]),db2)
...
class jump_Tests(unittest.TestCase):
    ...
    def test_dx(self):
        j = jump(0,0,or_1,or_2,np.array([0.,1.,0.]),np.array([0.,0.,0.]),np.array([0.,-1.,0.]),1,1)
        self.assertTrue(np.allclose(j.dx,j.R_d-j.R_s)
                       )
    def test_add(self):
    ...
    def test_gop(self):
        j3 = jump(0,0,or_1,or_2,np.array([0.,1.,0.]),np.array([0.,0.,1.]),np.array([0.,-1.,1.]),1,1)
        symm_jump_list=[]
        for g in crys.G:
